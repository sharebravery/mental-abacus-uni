...
const genFn = computed(() => {
  return () => GenerateArithmeticExpression.generateAdditionOrSubtractionExpression(params.option)
})

const resultFn = computed(() => {
  return (nums: number[]) => GenerateArithmeticExpression.calculateArrayExpression(nums, OperationType.加)
})

onLoad((option: any) => {
  const { type } = option || {}
  title.value = ExpressionType[Number(type)]
  params.option.expressionType = Number(type)
  uni.setNavigationBarTitle({ title: title.value })
})
</script>

<template>
  <view v-if="state === 'stop'" class="add-sub-box flex">
    ...
    <view class="add-sub-content">
      ...
      <view class="c2-content">
        <uni-title class="c2-title" type="h1" :title="title" align="center " />
    ...
    <view class="c2-footer">
      <text class="list-icon bg-#fe9e17" @click="state = 'start'">
        开始
      </text>
    </view>
  </view>

  <c-practice v-else :type="params.type" :speed="params.speed" :gen-fn="genFn" :result-fn="resultFn" :back="() => state = 'stop'" />
</template>
